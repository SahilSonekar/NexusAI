...
**Nexus AI** solves this with **Rutu**, an AI-powered virtual support assistant that understands natural language queries and autonomously resolves customer issues. Instead of relying solely on the language model's internal knowledge, Rutu combines LLM reasoning with **live business data** pulled straight from a MySQL database.
...
- For general company policies, product documentation, warranties, and FAQs, Rutu uses **Direct Context Injection** — official business documentation supplied directly within the model's system prompt.
...
* 🤖 AI-powered virtual customer support assistant ("Rutu")
...
Customers interact with Rutu using natural language, just like speaking to a human support representative — e.g. *"Where is my order?"*, *"Can I request a refund?"*, *"What is my warranty period?"*. Rutu determines intent, decides whether live data is required, invokes backend tools when necessary, and generates an accurate, conversational reply.
...
### 🚚 Delivery Tracking
Rutu retrieves shipment status, current location, estimated delivery date, delay reason, and carrier information before responding.
...
### 💬 Conversation Memory
Every conversation is stored in the database. Before processing a new message, prior history is reconstructed so Rutu keeps context across multiple turns (e.g. resolving "it" to the customer's most recently discussed order).
...
2. **Conversation History** — Django retrieves previous messages so Rutu understands context (e.g. "it" referring to a previously mentioned order).
3. **System Prompt Construction** — Django builds a prompt containing Rutu's identity, company policies, warranty/refund rules, product docs, conversation history, and the latest message.
...
Rutu: Your order #ORD-10245 has been shipped and is currently in transit.
...
Rutu: I checked your purchase history.
...
Rutu: For optimal cooling performance, we recommend cleaning the air filter
...
Rutu: Yes. Your order was dispatched yesterday and is currently with
BlueDart Logistics.
